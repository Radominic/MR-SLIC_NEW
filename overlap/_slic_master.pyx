   ...
   '''     
    ...
    with nogil: 
        for i in range(0,n_node):
            ...
            for z in range(z_min,z_min+distance[i].shape[0]):
                for y in range(y_min,y_min+distance[i].shape[1]):
                    for x in range(0, width):
                        if distanceO[z,y,x] > distance[i,z,y-y_min,x]:
                            distanceO[z,y,x] = distance[i,z,y-y_min,x]
                            nearest_segmentsO[z,y,x] = nearest_segments[i,z,y-y_min,x]
                ...
                for y in range(y_min,y_min+index[1][i]):
                    for x in range(0, width):
                        if distanceO[z,y,x] > distance[temp+y-y_min,x]:
                            ...
                            nearest_segmentsO[z,y,x] = nearest_segments[temp+y-y_min,x]
            temp += index[1][i]
